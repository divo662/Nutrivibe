...
/**
 * NutriVibe AI Testing Script for Groq (CommonJS Version)
...
async function callGroqAPI(model, messages, temperature = 0.7) {
  try {
    ...
    if (typeof fetch === 'undefined') {
      console.error('❌ Fetch is not available. You need Node.js 18+ or to install node-fetch');
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      console.error('💡 Tip: You might need to install node-fetch or use Node.js 18+');
    }
    return null;
  }
}
...
async function runAllTests() {
  console.log('🚀 NutriVibe AI Testing Suite for Groq (CommonJS Version)');
  console.log('='.repeat(80));
